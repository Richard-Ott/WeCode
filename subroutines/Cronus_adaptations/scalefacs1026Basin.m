...
sf.SLeth=1;
sf.P=sp1026.P;
sf.elevation=sp1026.elevation;  % these values are not used for the scaling
sf.longitude=sp1026.longitude;  % these values are not used for the scaling
sf.latitude=sp1026.latitude;    % these values are not used for the scaling
...
raster_lat = linspace(raster_lat(2),raster_lat(1),DEM.size(1));
raster_lat = raster_lat';
raster_lon = linspace(raster_lon(1),raster_lon(2),DEM.size(2));

raster_lat = repmat(raster_lat,1,DEM.size(2));
raster_lon = repmat(raster_lon,DEM.size(1),1);
